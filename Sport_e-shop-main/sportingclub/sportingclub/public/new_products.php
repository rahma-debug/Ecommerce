	...
	<div class="bg0 m-t-23 p-b-140">
		<div class="container">
			...
			<div class="row isotope-grid">
				...
				//$recent_date=
				if (isset($_GET["id"])) {
					$query1 = query(" SELECT * FROM product WHERE category = '{$row['label']}' AND DATEDIFF(NOW(),addeddate) <=30  ORDER BY addeddate DESC");
				} else {
					$query1 = query(" SELECT * FROM product WHERE DATEDIFF(NOW(),addeddate) <=30 ORDER BY addeddate DESC");
				}

				confirm($query1);
				while ($row1 = fetch_array($query1)) :
				?>
					<div class="col-sm-6 col-md-4 col-lg-3 p-b-35 isotope-item women label-new" data-label="New">
						<!-- Block2 -->
						<div class="block2">
			...
			<div>
				<center>
					<a class="stext-115 cl1 size-213 p-t-18"><?php if (mysqli_num_rows($query1) == 0) echo "Désolé! Pas d'articles récents pour cette catégorie pour le moment." ?></a>
				</center>
				<div class='row'>
