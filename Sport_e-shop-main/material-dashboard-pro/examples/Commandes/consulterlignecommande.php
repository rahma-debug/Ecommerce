          ...
          <li class="nav-item ">
            <a class="nav-link" href="../Calendar.php">
              ...
              <p> Calendar </p>
                                        ...
                                        $stmt = 'SELECT status From orders WHERE id="'.$commande.'";';
                                        $result = $idcom->query($stmt);
      ...
      <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            ...
            <a class="navbar-brand" href="#pablo">Détails de la commande <?php echo $commande ?></a>
          </div>
        </div>
      </nav>
      <div class="content">
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary card-header-icon">
                  ...
                  <h4 class="card-title">Détails de la commande <?php echo $commande ?></h4>
                </div>
                <div class="card-body">
                  <div class="toolbar">
                  ...
                  <div class="material-datatables">
                    ...
                    mysqli_set_charset($idcom1, "utf8");
                    $sql = 'SELECT quantity  FROM product  where id in (select product_id from order_line where order_id = "'.$commande.'" ) ; ';
                    $result = $idcom1->query($sql);
                    $quantitystock=$result->fetch_array(MYSQLI_NUM);
                   
                    
                    $idcom = new mysqli("localhost","root","","Sportingclub");
                    mysqli_set_charset($idcom, "utf8");
                    $req = $idcom->prepare('SELECT l.id,l.product_id,l.product_name,l.product_price,l.product_quantity,l.line_total  FROM order_line l  where l.order_id ='.$commande.' ; ');
                    $req->execute();
                    $req->bind_result($id,$id_produit,$nom_produit,$prix_produit,$quantity_demandée,$total_line);
                    while ($req->fetch()) {
                            echo'
                                  <tr>
                                      <td>'.$id.'</td>
                                      ...
                                      <td>'.$nom_produit.'</td>
                                      <td>'.$prix_produit.'</td>
                                      <td>'.$quantity_demandée.'</td>
                                      <td>'.$total_line.'</td>';
                               
                             if ($quantitystock[0]<$quantity_demandée){
                              if ($quantitystock[0]==0){
                              echo'<td style="color: red;font-weight: bold;">Produit hors stock</td>';}
                              else{
                                echo'<td style="color: red;font-weight: bold;">Quantitée non disponible</td>';
                              }
                              $disponible=0;
                              echo'<script> document.getElementById("button1").disabled = true;</script>';
                             }
                             else {
                              echo'<td style="color: green;font-weight: bold;">Quantitée disponible</td>';
              ...
              <div align="right">
                ...
                <?php if ($status[0] != "Livré" && $status[0] != "Confirmé"){
                if ($disponible == 1)
                {echo'<table><tr><td class="text-right"><form id="confirmer'.$commande.'" action="confirmer.php" method="POST" onsubmit=" return confirm(\'voulez-vous vraiment confirmer cette commande ? Cette action est non réversible et va engendrer une décrémentation des quantitées des produits confirmés dans la base !\') ">
                        <input type="text" class="form-control"  name="commande" value="'.$commande.'" style="display:none;">
                        <button type="submit" title="Confirmer la commande" class="btn btn-success">Confirmer</button>
                        </form></td>';}
                else {echo'<table><tr><td class="text-right"><form id="confirmer'.$commande.'" action="confirmer.php" method="POST" onsubmit=" return confirm(\'voulez-vous vraiment confirmer cette commande ? Cette action est non réversible et va engendrer une décrémentation des quantitées des produits confirmés dans la base !\') ">
                        <input type="text" class="form-control"  name="commande" value="'.$commande.'" style="display:none;">
                        <button type="submit" title="Confirmer la commande" class="btn btn-success disabled">Confirmer</button>
                        </form></td>';}
      
                echo'
                <td class="text-right"><form id="annuler'.$commande.'" action="rejeter.php" method="POST" onsubmit=" return confirm(\'voulez-vous vraiment rejeter cette commande ?\') ">
                  <input type="text" class="form-control"  name="commande" value="'.$commande.'" style="display:none;">
                  <button type="submit" title="Rejeter la commande" class="btn btn-danger">Rejeter</button>
                  </form></td></tr></table>';} ?>
              </div>
            </div>
          </div>
